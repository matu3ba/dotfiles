...
struct TestResult {
  enum class Status : uint8_t { NotRun = 0, Error, Passed, Failed, Skipped } status;
  std::string Info;
...
enum class TestType : uint8_t { Placeholder = 0, UnitTest, CrashTest, TestGroup };
...
struct TestClass {
  ...
  void AddTestFn(char const *TestFnName, std::function<int32_t()> test_fn, TestType test_ty) {
  ...
  void CheckAndAddTests(std::function<void()> check_fn) { check_fn(); }
  ...
  inline static void PrintEqFail(FILE *const cstream, char const *file, uint32_t line, char const *testname,
                                 char const *reference, char const *value) {
    fprintf(cstream, "%s:%d %s failed,\nexpected '%s' got '%s'\n", file, line, testname, reference, value);
  }
...
// #endif // ALLOW_BAD_PRACTICE
  ...
  static inline int32_t TestEqual(char const *testname, Ty1 const &value, Ty2 const &reference, char const *file,
                                  uint32_t const line) {
    if (!(value == reference)) {
...
#define TEST_EQUAL(A, B)                                                                   \
  {                                                                                        \
    int32_t res = TestClass::TestEqual(#A " == " #B, (A), (B), __FILE_NAME__, (__LINE__)); \
    if (res != 0)                                                                          \
      return res;                                                                          \
  }                                                                                        \
  do {                                                                                     \
