...
command -v 'ssh' 1> /dev/null || (echo "please install ssh"; exit 1)

CWD=$(pwd)
if test "${CWD}" != "${HOME}"; then echo "must run from ${HOME}"; exit 1; fi

if test $# -lt 3 || test $# -gt 5; then echo "Usage:
$0 \$1 \$2 [\$3]
  \$1: target user
  \$2: target ip
  \$3: keyname
...
if test -z "${DUPLICATES_OK}"; then
  ...
  if test $? -eq 0; then
    echo "duplicate ip, exiting.."; exit 1
  fi
fi
grep ${KEYNAME} "${HOME}/.ssh/config"
if test $? -eq 0; then
  echo "duplicate keyname, exiting.."; exit 1
fi
