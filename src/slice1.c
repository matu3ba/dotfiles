...
// clang-format off
// vvvvvvvvvvvvvvvv Modify this to adjust fns
...
#define CSTR_SLICE_NEW_GENERATOR_DECL(NAME, TYPE)      \
    __attribute__((always_inline)) inline cstr_##NAME  \
        cstr_new_##NAME(TYPE *buf, size_t len)

// before inline cstr_##NAME
#define CSTR_SLICE_NEW_GENERATOR_IMPL(NAME, TYPE)      \
    __attribute__((always_inline)) inline cstr_##NAME  \
        cstr_new_##NAME(TYPE *buf, size_t len)         \
    {                                                  \
...
#define CSTR_DEFINE_SLICE(NAME, TYPE)          \
    ...
    CSTR_SLICE_NEW_GENERATOR_DECL(NAME, TYPE); \
    CSTR_SLICE_NEW_GENERATOR_IMPL(NAME, TYPE)
